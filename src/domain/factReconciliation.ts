...
const RECONCILIATION_SYSTEM_PROMPT = [
  ...
  // D037 第三枪真机暴露的形态：用户两份简历对同一段时期用了两种写法（一份把两段实习分开写，
  // 一份合起来写）。合并版那条的雇主字段本身就含两个雇主，于是上一条 SAMENESS 规则正确地拒绝了关联，
  // 但结果是库里同一段时期留了两版、系统认不出它们在讲同一段历史。
  // 这一条只补这一种形态，且**只许判 conflict**：两版都是用户自己写的，谁为准归用户（D036 §四开放项）；
  // 判 merge/supplement 会让 reconstructMergedValue 拼接两段原文，产出用户没写过的东西。
  'COMPOSITE-RECORD RULE (covers one narrow form only, and never relaxes the rule above): sometimes the person described the same stretch of their history in two different ways, where one version bundles several employers or several projects into a single record while the other version covers only one of them. The precise trigger is: version A\'s own employer/project field literally names two or more distinct entities (for example "X／Y", "X and Y", "X, Y"), AND version B\'s entity is one of the entities named inside version A. In that specific situation the two versions are two competing descriptions of the same stretch of history written by the same person, so return "conflict" for them. NEVER return "merge" or "supplement" for this form, no matter how compatible the two texts look — the person themselves must decide which way of writing it stands, and joining the two texts would produce a description the person never actually wrote. This rule applies ONLY when one version\'s own entity field literally contains the other version\'s entity among several. If each version names exactly one entity and those entities are different, the previous rule governs and the answer is still "distinct".',
